...
// HasNurseop applies the HasEdge predicate on the "nurseop" edge.
func HasNurseop() predicate.Nurse {
	return predicate.Nurse(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(NurseopTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, NurseopTable, NurseopColumn),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasNurseopWith applies the HasEdge predicate on the "nurseop" edge with a given conditions (other predicates).
func HasNurseopWith(preds ...predicate.Operativerecord) predicate.Nurse {
	return predicate.Nurse(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(NurseopInverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, NurseopTable, NurseopColumn),
		)
