...
type ToolQuery struct {
	...
	// eager-loading edges.
	withFromtool *OperativerecordQuery
...
// QueryFromtool chains the current query on the fromtool edge.
func (tq *ToolQuery) QueryFromtool() *OperativerecordQuery {
...
//  WithFromtool tells the query-builder to eager-loads the nodes that are connected to
// the "fromtool" edge. The optional arguments used to configure the query builder of the edge.
func (tq *ToolQuery) WithFromtool(opts ...func(*OperativerecordQuery)) *ToolQuery {
	query := &OperativerecordQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withFromtool = query
	return tq
}
...
func (tq *ToolQuery) sqlAll(ctx context.Context) ([]*Tool, error) {
	...
	if query := tq.withFromtool; query != nil {
		...
		query.Where(predicate.Operativerecord(func(s *sql.Selector) {
			s.Where(sql.InValues(tool.FromtoolColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.tool_name
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "tool_name" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "tool_name" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Fromtool = append(node.Edges.Fromtool, n)
		}
	}
