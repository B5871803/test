...
func (nc *NurseCreate) AddNurseopIDs(ids ...int) *NurseCreate {
	nc.mutation.AddNurseopIDs(ids...)
	return nc
}

// AddNurseop adds the nurseop edges to Operativerecord.
func (nc *NurseCreate) AddNurseop(o ...*Operativerecord) *NurseCreate {
	ids := make([]int, len(o))
	for i := range o {
		ids[i] = o[i].ID
	}
	return nc.AddNurseopIDs(ids...)
}
...
func (nc *NurseCreate) createSpec() (*Nurse, *sqlgraph.CreateSpec) {
	...
	if nodes := nc.mutation.NurseopIDs(); len(nodes) > 0 {
