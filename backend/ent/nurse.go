...
type NurseEdges struct {
	// Nurseop holds the value of the nurseop edge.
	Nurseop []*Operativerecord
	// loadedTypes holds the information for reporting if a
	// type was loaded (or requested) in eager-loading or not.
	loadedTypes [1]bool
}

// NurseopOrErr returns the Nurseop value or an error if the edge
// was not loaded in eager-loading.
func (e NurseEdges) NurseopOrErr() ([]*Operativerecord, error) {
	if e.loadedTypes[0] {
		return e.Nurseop, nil
	}
	return nil, &NotLoadedError{edge: "nurseop"}
}
...
// QueryNurseop queries the nurseop edge of the Nurse.
func (n *Nurse) QueryNurseop() *OperativerecordQuery {
	return (&NurseClient{config: n.config}).QueryNurseop(n)
}
