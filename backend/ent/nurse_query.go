...
type NurseQuery struct {
	...
	// eager-loading edges.
	withNurseop *OperativerecordQuery
...
// QueryNurseop chains the current query on the nurseop edge.
func (nq *NurseQuery) QueryNurseop() *OperativerecordQuery {
...
//  WithNurseop tells the query-builder to eager-loads the nodes that are connected to
// the "nurseop" edge. The optional arguments used to configure the query builder of the edge.
func (nq *NurseQuery) WithNurseop(opts ...func(*OperativerecordQuery)) *NurseQuery {
	query := &OperativerecordQuery{config: nq.config}
	for _, opt := range opts {
		opt(query)
	}
	nq.withNurseop = query
	return nq
}
...
func (nq *NurseQuery) sqlAll(ctx context.Context) ([]*Nurse, error) {
	...
	if query := nq.withNurseop; query != nil {
		...
		query.Where(predicate.Operativerecord(func(s *sql.Selector) {
			s.Where(sql.InValues(nurse.NurseopColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node.Edges.Nurseop = append(node.Edges.Nurseop, n)
		}
	}
