...
type ToolEdges struct {
	...
	Fromtool []*Operativerecord
	// loadedTypes holds the information for reporting if a
	// type was loaded (or requested) in eager-loading or not.
	loadedTypes [1]bool
}

// FromtoolOrErr returns the Fromtool value or an error if the edge
// was not loaded in eager-loading.
func (e ToolEdges) FromtoolOrErr() ([]*Operativerecord, error) {
	if e.loadedTypes[0] {
		return e.Fromtool, nil
	}
	return nil, &NotLoadedError{edge: "fromtool"}
}
...
func (t *Tool) assignValues(values ...interface{}) error {
	...
	if value, ok := values[0].(*sql.NullString); !ok {
		return fmt.Errorf("unexpected type %T for field toolName", values[0])
	} else if value.Valid {
...
// QueryFromtool queries the fromtool edge of the Tool.
func (t *Tool) QueryFromtool() *OperativerecordQuery {
	return (&ToolClient{config: t.config}).QueryFromtool(t)
}
